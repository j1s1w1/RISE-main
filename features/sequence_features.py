#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Frozen ESM2 per-residue sequence feature extraction for RISE.

This module is responsible only for:
1. loading a local ESM2 model;
2. extracting per-residue representations;
3. validating and caching sequence features.

The extracted representations are consumed by the trainable sequence encoder
defined in the RISE training code.
"""
...
if __package__:
    ...
    # python features/sequence_features.py ...
    import sys
...
def safe_torch_load(
    ...
    """Load a feature cache across supported PyTorch versions."""
    try:
        return torch.load(
            fp,
            map_location=device,
...
def load_esm2_model(
    ...
    """Load a local HuggingFace ESM2 model and freeze its parameters."""
    model_path = Path(esm_model_path).expanduser()

    if not model_path.exists():
        raise FileNotFoundError(
            f"ESM2 model path not found: {model_path}"
        )
    ...
    print("Loading local ESM2 model ...")
...
def validate_sequence_feature_cache(
    ...
    missing = sorted(
        required.difference(data.keys())
    )

    if missing:
        raise ValueError(
            f"Sequence feature cache is missing keys: {missing}"
        )
    ...
    if len(set(lengths.values())) != 1:
        raise ValueError(
            f"Inconsistent sequence cache lengths: {lengths}"
        )
    ...
    for index, (sequence, embedding) in enumerate(
        ...
        if (
            ...
            raise ValueError(
                f"Embedding {index} must be a rank-2 tensor, "
...
def extract_token_embeddings(
    ...
    """Extract frozen ESM2 per-residue representations.

    ``repr_layer`` is kept as an interface parameter so existing configuration
    files and commands remain valid. The HuggingFace ``EsmModel`` interface
    returns the model hidden representation used by this pipeline.
    """
    del repr_layer
    ...
    if cache_path.is_file():
        print(
            f"Sequence embeddings already cached: {cache_path}"
        )
        cached = safe_torch_load(
            cache_path,
            "cpu",
        )
        validate_sequence_feature_cache(
            cached,
            records,
        )
        return cached
    ...
    if not records:
        raise ValueError(
            "Cannot extract sequence features from an empty record list"
        )

    if esm_batch_size < 1:
        raise ValueError(
            f"esm_batch_size must be >= 1, got {esm_batch_size}"
        )

    print(
        "Extracting frozen ESM2 per-residue representations: "
    ...
    while index < len(records):
        ...
        try:
            ...
            for batch_index, record in enumerate(
                ...
                # The first token is the leading special token.
                embedding = (
                    last_hidden[
                        batch_index,
                        1 : 1 + sequence_length,
                ...
                if embedding.shape[0] != sequence_length:
                    raise RuntimeError(
                        "ESM2 token length mismatch for "
                        f"id={record['id']!r}: "
            ...
            if (
                index == len(records)
                or (
                    index // max(current_batch_size, 1)
                ) % 10 == 0
            ):
                elapsed = time.time() - start_time
                print(
                    f"  ESM2 representations: "
                    f"{index}/{len(records)} | "
            ...
            print(
                "CUDA OOM: "
                f"esm_batch_size {current_batch_size} "
                f"-> {new_batch_size}"
            )
    ...
    print(
        f"Saved ESM2 sequence features: {cache_path}"
    )
...
# Public name used by the training/evaluation pipeline.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Extract frozen ESM2 per-residue features "
            "for AMP/non-AMP FASTA files."
        )
    )
